...
import com.jspstudio.ex83retrofit2.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    // 2. Retrofit - OkHttp를 기반으로 개량한 라이브러리. sqareup 회사가 보유 [ 현재 가장 많이 사용됨 ] - 현재는 version2로 사용
    // 3. Volley - Google 에서 제작함. 그러나 업데이트 지원정책에 혼선이 발생 [ 현재 사용은 조금 애매함 ]
    ...
    void clickBtn9(){ //// converter-scalars 라이브러리도 추가해서 사용 ////
        // 서버 응답결과를 그냥 Text로 받아오기. json 문자열이 아닐때는 반드시 필요한 기술
        // json이 아니므로 GsonConverter를 사용하지 않음
        // 단순하게 글씨로 결과를 받는 Converter : ScalarsConverter [ 라이브러리 추가 필요 ]

        Retrofit.Builder builder= new Retrofit.Builder();
        builder.baseUrl("http://jspstudio.dothome.co.kr/");
    ...
    void clickBtn7(){
        // POST 방식으로 데이터를 따로따로 보내기 : 마치 GET방식의 @Query처럼
        int no= 2;
        String title= "bbb";
    ...
    void clickBtn6(){
        // POST 방식으로 데이터 보내기 - 보통은 보낼데이터를 가진 객체로 만들어져 있는 경우가 많음 [ Retrofit이 자동으로 객체를 json문자열로 변환하여 POST함 ]
    ...
    void clickBtn5(){
        ...
        datas.put("title", "안녕하세요");
        datas.put("msg", "반가워용");

        // 1. Retrofit 객체 생성 - RetrofitHelper 이용
        Retrofit retrofit= RetrofitHelper.getRetrofitInstance();

        // 2,3. 추상메소드 설계 및 인터페이스객체 생성
        RetrofitService retrofitService= retrofit.create(RetrofitService.class);
    ...
    void clickBtn4(){
        // GET 방식 데이터 전달 + 파일경로도 지정 [ 2번 + 3번 버튼 예제 ]

        String title= "Good";
        String msg= "Very Good";

        // 1. Retrofit 객체 생성 - 항상 4줄씩 쓰기가 번거로움. 그래서 별도 클래스(RetrofitHelper.class)에 기능으로 만들기
        Retrofit retrofit= RetrofitHelper.getRetrofitInstance();

        // 2,3. 인터페이스 설계 및 객체 생성
        RetrofitService retrofitService= retrofit.create(RetrofitService.class);

        // 4. 추상메소드 호출
        Call<Item> call= retrofitService.getMethodTest2("getTest.php",title, msg);
    ...
    void clickBtn3(){
        ...
        // 4. RetrofitService객체의 추상메소드 호출하면서 서버에 보낼 데이터를 파라미터로 전달
        Call<Item> call= retrofitService.getMethodTest(title, message);

        // 5. 네트워크 작업 시작 및 응답 받도록 callback 객체 등록
        call.enqueue(new Callback<Item>() {
            @Override
            public void onResponse(Call<Item> call, Response<Item> response) {
                Item item= response.body();
    ...
    void clickBtn(){
        ...
        // Retrofit2 Library를 이용하여 서버에서 json 데이터를 읽어와서 Item 객체로 곧바로 파싱

        // 1. Retrofit 객체 생성
        Retrofit.Builder builder= new Retrofit.Builder();
        builder.baseUrl("http://jspstudio.dothome.co.kr/"); // 마지막에 / 는 써도되고 안써도됨
        builder.addConverterFactory(GsonConverterFactory.create());
        Retrofit retrofit= builder.build();
        ...
        // 4. 위에서 만든 이 서비스객체의 추상메소드를 호출하여 실제 서버작업을 수행하는 코드를 가진
        //    Call 이라는 객체를 리턴받기
